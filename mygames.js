...
await page.goto(url);
       ...
       // let title = document.body.querySelector('#img-responsive cover_small cover_small').innerText;
        ...
        Array.from(document.querySelectorAll('div.game_cover img'))
        .map(logo =>logo.src)
        )

        const title = await page.evaluate(() =>
        Array.from(document.querySelectorAll('div.media-body a'))
        .map(title =>title.href)
        )

        const gameDate = await page.evaluate(() =>
        Array.from(document.querySelectorAll('div.text-muted time'))
        .map(gameDate =>gameDate.time)
        )
